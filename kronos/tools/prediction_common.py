...
MODEL_CONFIGS = {
    "mini":  {"model": "NeoQuasar/Kronos-mini",  "tokenizer": "NeoQuasar/Kronos-Tokenizer-2k",  "context": 2048},
    "small": {"model": "NeoQuasar/Kronos-small", "tokenizer": "NeoQuasar/Kronos-Tokenizer-base", "context": 512},
    "base":  {"model": "NeoQuasar/Kronos-base",  "tokenizer": "NeoQuasar/Kronos-Tokenizer-base", "context": 512},
    "large": {"model": "NeoQuasar/Kronos-large", "tokenizer": "NeoQuasar/Kronos-Tokenizer-base", "context": 512},
}


def load_model(model_path=None, device="cuda:0", model_type="mini"):
    print("Loading Kronos model...")
    try:
        cfg = MODEL_CONFIGS.get(model_type)
        if cfg is None:
            print(f"Unknown model variant '{model_type}'. Choose from: {list(MODEL_CONFIGS.keys())}")
            return None
        ...
        tokenizer = KronosTokenizer.from_pretrained(cfg["tokenizer"], **kwargs)
        model = Kronos.from_pretrained(cfg["model"], **kwargs)
        tokenizer.eval()
        model.eval()
        predictor = KronosPredictor(model, tokenizer, device=device, max_context=cfg["context"])
        print(f"Model loaded successfully. (variant={model_type}, context={cfg['context']})")
        return predictor
    except Exception as e:
        print(f"Failed to load model: {e}")
        print("Prediction functionality unavailable, please check model installation.")
        return None


def make_prediction(df, predictor, pred_horizon, n_predictions, step_minutes=5):
    ...
    new_timestamps = []
    t = last_timestamp
    while len(new_timestamps) < pred_horizon:
...
def create_plot(hist_df, close_preds_df, volume_preds_df, repo_path, symbol, step_minutes=5):
    ...
    ax1.set_title(f'{symbol} - Probabilistic Price & Volume Forecast', fontsize=16, weight='bold')
    ...
    fig.tight_layout()
    chart_path = repo_path / 'prediction_chart.png'
